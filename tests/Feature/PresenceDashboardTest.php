...
it('renders the selected year with summaries components and only touching trips', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('summary.spt_current_year_days', 10)
            ->where('summary.spt_weighted_total_sixths', 84)
            ->where('summary.spt_weighted_total', '14')
            ->where('summary.spt_meets_31_day_requirement', false)
            ->where('summary.spt_meets_183_day_requirement', false)
            ->where('summary.spt_met', false)
